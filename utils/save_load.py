...
def load_distillation_model(model, pretrained_model):
    ...
    load_dygraph_pretrain(teacher, pretrained_path=pretrained_model[0])
    logger.info("Finish initing teacher model from {}".format(
        pretrained_model))
    # load student model
    if len(pretrained_model) >= 2:
        load_dygraph_pretrain(student, pretrained_path=pretrained_model[1])
...
def init_model(config,
    ...
    if pretrained_model:
        if use_distillation:
            ...
            load_dygraph_pretrain(net, pretrained_path=pretrained_model)
